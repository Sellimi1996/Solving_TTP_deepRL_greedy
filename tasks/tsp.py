...
class TSPDataset(Dataset):
    ...
    def __init__(self, size=50, num_samples=1e6, seed=None, weightmax=1.92):
        ...
        #weightmax = torch.sum(self.dataset[:,:,2])
        self.capacity= weightmax
...
def reward(static, tour_indices, capacity):
    """
    Parameters
    ----------
    ...
    #tour_len = torch.sqrt(torch.sum(torch.pow(y[:, :1] - y[:, 1:-1], 2)/(1-y[:, -1:]*0.9/capacity), dim=2))
